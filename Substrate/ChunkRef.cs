...
public class ChunkRef : IChunk
{
    ...
    public void SetLocation(int x, int z)
    {
        ...
        var c = _container.SetChunk(relX, relZ, GetChunk());
    ...
    {
        if (!container.ChunkExists(cx, cz)) return null;
    ...
    /// </summary>
    /// <remarks>
    ///     This function returns the reference to the chunk stored in the chunk container.  Because the ChunkRef
    ///     simultaneously gives up
    ///     its "ownership" of the Chunk, the container will not consider the Chunk dirty even if it is modified.  Attempting
    ///     to use the ChunkRef after
    ///     releasing its internal reference will query the container for a new reference.  If the chunk is still cached, it
    ///     will get the same reference
    ///     back, otherwise it will get an independent copy.  Chunks should only be taken from ChunkRefs to transfer them to
    ///     another ChunkRef, or
    ...
    /// <summary>
    ///     Replaces the underlying physical chunk with a different one, updating its physical location to reflect the
    ///     ChunkRef.
    /// </summary>
    /// <remarks>
    ...
    {
        var cr = _container.GetChunkRef(_cx + relx, _cz + relz);
        if (cr != null) return cr.Blocks;
    ...
    {
        var x = X * _blocks.XDim + lx;
        var z = Z * _blocks.ZDim + lz;
