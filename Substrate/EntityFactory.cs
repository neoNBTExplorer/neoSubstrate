...
/// </summary>
/// <remarks>
///     This factory allows specific <see cref="TypedEntity" /> objects to be generated as an NBT tree is parsed.  New
///     types can be
///     registered with the factory at any time, so that custom <see cref="TypedEntity" /> types can be supported.  By
...
public class EntityFactory
{
    ...
    public static IEnumerable<KeyValuePair<string, Type>> RegisteredEntities
    {
        get
        {
            foreach (var kvp in _registry) yield return kvp;
        }
    }
    ...
    /// <returns>
    ///     A new instance of a concrete <see cref="TypedEntity" /> type, or null if no type was registered with the given
    ///     name.
    /// </returns>
    public static TypedEntity Create(string type)
    {
        Type t;
        if (!_registry.TryGetValue(type, out t)) return null;
    ...
    /// </summary>
    /// <param name="tree">
    ///     A <see cref="TagNodeCompound" /> representing a single Entity, containing an 'id' field of the
    ...
    {
        TagNode type;
        if (!tree.TryGetValue("id", out type)) return null;

        Type t;
        if (!_registry.TryGetValue(type.ToTagString(), out t)) return null;

        var te = Activator.CreateInstance(t) as TypedEntity;
    ...
    public static TypedEntity CreateGeneric(TagNodeCompound tree)
    {
        ...
        var te = new TypedEntity(type.ToTagString().Data);
    ...
    /// <param name="type">The name that a concrete <see cref="TypedEntity" /> type was registered with.</param>
    /// <returns>
    ///     The <see cref="Type" /> of a concrete <see cref="TypedEntity" /> type, or null if no type was registered with
    ///     the given name.
    /// </returns>
    ...
    /// <summary>
    ///     Registers a new concrete <see cref="TypedEntity" /> type with the <see cref="EntityFactory" />, binding it to a
    ///     given name.
    /// </summary>
