...
public abstract class BlockManager : IVersion10BlockManager, IBlockManager
{
    ...
    public AlphaBlock GetBlock(int x, int y, int z)
    {
        ...
        if (cache == null || !Check(x, y, z)) return null;
    ...
    {
        cache = GetChunk(x, y, z);
        if (cache == null || !Check(x, y, z)) return new AlphaBlockRef();
    ...
    {
        cache = GetChunk(x, y, z);
        if (cache == null || !Check(x, y, z)) return;
    ...
    public void SetID(int x, int y, int z, int id)
    {
        ...
        var autolight = cache.Blocks.AutoLight;
        var autofluid = cache.Blocks.AutoFluid;
        var autoTileTick = cache.Blocks.AutoTileTick;
    ...
    public int GetHeight(int x, int z)
    {
        ...
        if (cache == null || !Check(x, 0, z)) return 0;
    ...
    {
        cache = GetChunk(x, 0, z);
        if (cache == null || !Check(x, 0, z)) return;
    ...
    {
        cache = GetChunk(x, y, z);
        if (cache == null || !Check(x, y, z)) return 0;
