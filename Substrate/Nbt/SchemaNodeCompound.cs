...
{
    private readonly List<SchemaNode> _subnodes;
    ...
    /// <summary>
    ///     Constructs a new <see cref="SchemaNodeCompound" /> representing a <see cref="TagNodeCompound" /> named
    ///     <paramref name="name" />.
    /// </summary>
    ...
    /// <summary>
    ///     Constructs a new <see cref="SchemaNodeCompound" /> representing a <see cref="TagNodeCompound" /> named
    ///     <paramref name="name" /> matching the given schema.
    /// </summary>
    /// <param name="name">The name of the corresponding <see cref="TagNodeCompound" />.</param>
    /// <param name="subschema">
    ///     A <see cref="SchemaNodeCompound" /> representing a schema to verify against the corresponding
    ///     <see cref="TagNodeCompound" />.
    /// </param>
        ...
        : base(name)
    {
        ...
        var schema = subschema as SchemaNodeCompound;
        if (schema == null) return;

        foreach (var node in schema._subnodes) _subnodes.Add(node);
    }
    ...
    public SchemaNodeCompound MergeInto(SchemaNodeCompound tree)
    {
        ...
        {
            var f = tree._subnodes.Find(n => n.Name == node.Name);
            if (f != null) continue;
            tree.Add(node);
        }
    ...
    /// </summary>
    /// <returns>
    ///     A <see cref="TagNodeCompound" /> with a sensible default value.  A default child <see cref="TagNode" /> is
    ///     created for every <see cref="SchemaNode" /> contained in this <see cref="SchemaNodeCompound" />.
    /// </returns>
    public override TagNode BuildDefaultTree()
    {
        var list = new TagNodeCompound();
        foreach (var node in _subnodes) list[node.Name] = node.BuildDefaultTree();
    ...
    /// <summary>
    ///     Copies all child <see cref="SchemaNode" /> objects of this node to a compatible one-dimensional array, starting at
