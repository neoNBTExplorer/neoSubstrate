...
/// </summary>
/// <remarks>
///     This factory allows specific <see cref="TileEntity" /> objects to be generated as an NBT tree is parsed.  New types
///     can be
///     registered with the factory at any time, so that custom <see cref="TileEntity" /> types can be supported.  By
...
public class TileEntityFactory
{
    ...
    /// <returns>
    ///     A new instance of a concrete <see cref="TileEntity" /> type, or null if no type was registered with the given
    ///     name.
    /// </returns>
    ...
    /// </summary>
    /// <param name="tree">
    ///     A <see cref="TagNodeCompound" /> representing a single Tile Entity, containing an 'id' field of the
    ///     Tile Entity's registered name.
    /// </param>
    /// <returns>
    ///     A new instance of a concrete <see cref="TileEntity" /> type, or null if no type was registered with the given
    ///     name.
    ...
    public static TileEntity Create(TagNodeCompound tree)
    {
        ...
        var te = Activator.CreateInstance(t) as TileEntity;
    ...
    public static TileEntity CreateGeneric(TagNodeCompound tree)
    {
        ...
        if (!_registry.TryGetValue(type, out t)) t = typeof(TileEntity);

        var te = Activator.CreateInstance(t, true) as TileEntity;
    ...
    /// <param name="type">The name that a concrete <see cref="TileEntity" /> type was registered with.</param>
    /// <returns>
    ///     The <see cref="Type" /> of a concrete <see cref="TileEntity" /> type, or null if no type was registered with
    ///     the given name.
    /// </returns>
    ...
    /// <summary>
    ///     Registers a new concrete <see cref="TileEntity" /> type with the <see cref="TileEntityFactory" />, binding it to a
    ///     given name.
    /// </summary>
