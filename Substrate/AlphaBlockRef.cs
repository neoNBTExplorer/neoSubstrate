...
/// <remarks>
///     <para>
///         The <see cref="AlphaBlockRef" /> type provides a reasonably lightweight reference to an individual block in a
///         <see cref="AlphaBlockCollection" />.  The <see cref="AlphaBlockRef" /> does not store any of the data itself.
///         If the referenced
///         block in the <see cref="AlphaBlockCollection" /> is updated externally, those changes will be automatically
///         reflected in the
///         <see cref="AlphaBlockRef" />, and any changes made via the <see cref="AlphaBlockRef" /> will be applied
///         directly to the corresponding
///         block within the <see cref="AlphaBlockCollection" />.  Such changes will also set the dirty status of the
///         <see cref="AlphaBlockCollection" />,
///         which can make this type particularly useful.
///     </para>
///     <para>
///         Despite being lightweight, using an <see cref="AlphaBlockRef" /> to get and set block data is still more
///         expensive then directly
///         getting and setting data in the <see cref="AlphaBlockCollection" /> object, and can be significantly slow in a
///         tight loop
///         (<see cref="AlphaBlockCollection" /> does not provide an interface for enumerating <see cref="AlphaBlockRef" />
///         objects specifically
///         to discourage this kind of use).
///     </para>
///     <para>
///         <see cref="AlphaBlockRef" /> objects are most appropriate in cases where looking up an object requires
///         expensive checks, such as
///         accessing blocks through a derived <see cref="BlockManager" /> type with enhanced block filtering.  By getting
///         an <see cref="AlphaBlockRef" />,
///         any number of block attributes can be read or written to while only paying the lookup cost once to get the
///         reference.  Using the
///         <see cref="BlockManager" /> (or similar) directly would incur the expensive lookup on each operation.  See
///         NBToolkit for an example of this
///         use case.
///     </para>
///     <para>
///         Unlike the <see cref="AlphaBlock" /> object, this type exposed access to context-dependent data such as
///         lighting.
///     </para>
/// </remarks>
public struct AlphaBlockRef : IVersion10BlockRef
{
    ...
    public int ID
    {
        get => _collection.GetID(_index);
        set => _collection.SetID(_index, value);
    }
    ...
    public int Data
    {
        get => _collection.GetData(_index);
        set => _collection.SetData(_index, value);
    }
    ...
    {
        get => _collection.GetBlockLight(_index);
        set => _collection.SetBlockLight(_index, value);
    }
    ...
    public int SkyLight
    {
        get => _collection.GetSkyLight(_index);
        set => _collection.SetSkyLight(_index, value);
    }
    ...
    /// </summary>
    /// <returns>
    ///     The <see cref="TileEntity" /> attached to this block, or null if the block type does not require a Tile
    ///     Entity.
    /// </returns>
    ...
    {
        get => _collection.GetTileTickValue(_index);
