...
public class AnvilWorld : NbtWorld
{
    ...
    private readonly Dictionary<string, ChunkCache> _caches;
    private readonly Dictionary<string, RegionChunkManager> _chunkMgrs;

    private readonly Dictionary<string, AnvilRegionManager> _regionMgrs;
    ...
    ///     Consider using the <see cref="RegionChunkManager" /> if you are interested in working with blocks.
    /// </remarks>
    ...
    public AnvilRegionManager GetRegionManager(string dim)
    {
        ...
        if (_regionMgrs.TryGetValue(dim, out rm)) return rm;
    ...
    {
        if (_caches.ContainsKey(dim)) return _caches[dim];
        return null;
    }
    ...
    {
        var world = new AnvilWorld().OpenWorld(path);
    ...
    {
        var world = new AnvilWorld().CreateWorld(path);
        world._prefCacheSize = cacheSize;
    ...
    {
        if (_dataMan != null) return _dataMan;
    ...
    private void OpenDimension(string dim)
    {
        ...
        var cc = new ChunkCache(_prefCacheSize);

        var rm = new AnvilRegionManager(path, cc);
        var cm = new RegionChunkManager(rm, cc);
    ...
    private AnvilWorld CreateWorld(string path)
    {
        ...
        var regpath = IO.Path.Combine(path, _REGION_DIR);
        if (!IO.Directory.Exists(regpath)) IO.Directory.CreateDirectory(regpath);
    ...
    internal static void OnResolveOpen(object sender, OpenWorldEventArgs e)
    {
        try
        {
            var world = new AnvilWorld().OpenWorld(e.Path);
            if (world == null) return;

            var regPath = IO.Path.Combine(e.Path, _REGION_DIR);
            if (!IO.Directory.Exists(regPath)) return;

            if (world.Level.Version < 19133) return;
