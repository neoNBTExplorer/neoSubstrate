...
{
    private static readonly SchemaNodeList _listSchema = new("", TagType.TAG_COMPOUND, Schema);
    ...
    /// <remarks>
    ///     The <paramref name="capacity" /> parameter does not necessarily indicate the true capacity of an item collection.
    ///     The player object, for example, contains a conventional inventory, a range of invalid slots, and then equipment.
    ///     Capacity in
    ///     this case would refer to the highest equipment slot.
    /// </remarks>
    ...
    {
        var ic = new ItemCollection(Capacity);
        foreach (var item in _items) ic[item.Key] = item.Value.Copy();
        return ic;
    }
    ...
    /// </summary>
    public static SchemaNodeCompound Schema { get; } = Item.Schema.MergeInto(new SchemaNodeCompound("")
    {
    ...
    {
        var ltree = tree as TagNodeList;
        if (ltree == null) return null;
    ...
    public TagNode BuildTree()
    {
        ...
        {
            var itemtree = item.Value.BuildTree() as TagNodeCompound;
