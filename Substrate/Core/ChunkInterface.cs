...
public interface IChunkContainer
{
    ...
    /// </summary>
    /// <remarks>
    ///     If a container does not support this property, it is expected to throw <see cref="ArgumentOutOfRangeException" />
    ...
    /// <remarks>
    ///     This is largely intended for internal use.  If an <see cref="IChunk" /> is assigned coordinates by an
    ///     <see cref="IChunkContainer" />, the interpretation of those coordinates is ambiguous.  This method ensures the
    ///     coordinate
    ...
    /// <param name="cz">The container-local Z-coordinate of a chunk.</param>
    /// <returns>
    ///     A <see cref="ChunkRef" /> for the given coordinates binding a <see cref="IChunk" /> to this container, or null if
    ...
    /// <param name="cz">The container-local Z-coordinate of a chunk.</param>
    /// <returns>
    ///     A <see cref="ChunkRef" /> for the newly created chunk if no previous chunk existed; a <see cref="ChunkRef" />
    ...
    /// <remarks>
    ///     <para>
    ///         The <see cref="IChunk" /> argument will be updated to reflect new global coordinates corresponding to
    ///         the given location in this container.  It is up to the developer to ensure that no competing
    ///         <see cref="ChunkRef" />
    ///         has a handle to the <see cref="IChunk" /> argument, or an inconsistency could develop where the chunk held by
    ///         the
    ///         other <see cref="ChunkRef" /> is written to the underlying data store with invalid coordinates.
    ///     </para>
    ///     <para>
    ///         The <see cref="ChunkRef" /> specification is designed to avoid this situation from occuring, but
