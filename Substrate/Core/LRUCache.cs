...
internal class LRUCache<TKey, TValue> : IDictionary<TKey, TValue>
{
    ...
    private readonly IndexedLinkedList<TKey> _index;

    public LRUCache(int capacity)
    {
        if (capacity <= 0) throw new ArgumentException("Cache capacity must be positive");
    ...
    {
        if (RemoveCacheValue != null) RemoveCacheValue(this, e);
    }
    ...
    {
        if (_data.ContainsKey(key)) throw new ArgumentException("Attempted to insert a duplicate key");
    ...
    {
        if (!_data.TryGetValue(key, out value)) return false;
