...
/// <remarks>
///     <para>
///         In general, you should prefer other types for accessing block data including <see cref="AlphaBlockRef" />,
///         <see cref="BlockManager" />, and the <see cref="AlphaBlockCollection" /> property of <see cref="IChunk" /> and
///         <see cref="ChunkRef" />.
///     </para>
///     <para>
///         You should use the <see cref="AlphaBlock" /> type when you need to copy individual blocks into a custom
///         collection or
...
///     </para>
///     <para>
///         A single <see cref="AlphaBlock" /> object may also provide a convenient way to paste a block into many
///         locations in
...
public class AlphaBlock : IDataBlock, IPropertyBlock, IActiveBlock, ICopyable<AlphaBlock>
{
    ...
    ///     <see cref="TileEntity" /> of the appropriate type will automatically be created.
    /// </remarks>
    ...
    public AlphaBlock(AlphaBlockCollection chunk, int lx, int ly, int lz)
    {
        ...
        Data = chunk.GetData(lx, ly, lz);

        var te = chunk.GetTileEntity(lx, ly, lz);
        if (te != null) _tileEntity = te.Copy();
    }
    ...
    {
        var block = new AlphaBlock(_id, Data);
        if (_tileEntity != null) block._tileEntity = _tileEntity.Copy();
    ...
    {
        var info1 = BlockInfo.BlockTable[old] as BlockInfoEx;
        var info2 = BlockInfo.BlockTable[value] as BlockInfoEx;

        if (info1 != info2)
        {
            if (info1 != null) _tileEntity = null;

            if (info2 != null) _tileEntity = TileEntityFactory.Create(info2.TileEntityName);
        }
    }
    ...
    {
        var info = BlockInfo.BlockTable[_id] as BlockInfoEx;
        if (info == null) throw new InvalidOperationException("The current block type does not accept a Tile Entity");
