...
/// </summary>
/// <remarks>
///     The <see cref="ChunkCache" /> class maintains a separate dictionary on the side for tracking
...
public class ChunkCache
{
    ...
    /// <remarks>
    ///     If the chunk does not already exist and the list is full, then the least-recently used chunk in the cache will be
    ///     evicted
    ...
    /// <remarks>
    ///     If the <see cref="ChunkRef" /> is found in the LRU-portion of the cache, it will be moved to the front of the
    ///     LRU list, making future eviction less likely.
    /// </remarks>
    public ChunkRef Fetch(ChunkKey key)
    {
        ChunkRef c;
        if (_dirty.TryGetValue(key, out c)) return c;
    ...
    ///     not be affected.  To clear dirty chunks as well, see <see cref="ClearDirty" />.
    /// </remarks>
    public void Clear()
    {
        _cache.Clear();
    ...
    {
        if (e.Value.IsDirty) _dirty[e.Key] = e.Value;
    }
