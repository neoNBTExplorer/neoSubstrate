...
{
    private static readonly Color[] _defaultColorIndex;

    private readonly Vector3[] _labIndex;

    private int _groupSize = 4;

    static MapConverter()
    {
        _defaultColorIndex = new[]
        {
    ...
    public MapConverter()
    {
        ColorIndex = new Color[256];
        _labIndex = new Vector3[256];
        _defaultColorIndex.CopyTo(ColorIndex, 0);

        RefreshColorCache();

        // Setup default block index
        BlockIndex = new ColorGroup[4096];
        for (var i = 0; i < BlockIndex.Length; i++) BlockIndex[i] = ColorGroup.Other;

        BlockIndex[BlockInfo.Grass.ID] = ColorGroup.Grass;
        BlockIndex[BlockInfo.TallGrass.ID] = ColorGroup.Grass;
        BlockIndex[BlockInfo.Sand.ID] = ColorGroup.Sand;
        BlockIndex[BlockInfo.Gravel.ID] = ColorGroup.Sand;
        BlockIndex[BlockInfo.Sandstone.ID] = ColorGroup.Sand;
        BlockIndex[BlockInfo.Lava.ID] = ColorGroup.Lava;
        BlockIndex[BlockInfo.StationaryLava.ID] = ColorGroup.Lava;
        BlockIndex[BlockInfo.Ice.ID] = ColorGroup.Ice;
        BlockIndex[BlockInfo.Leaves.ID] = ColorGroup.Leaves;
        BlockIndex[BlockInfo.YellowFlower.ID] = ColorGroup.Leaves;
        BlockIndex[BlockInfo.RedRose.ID] = ColorGroup.Leaves;
        BlockIndex[BlockInfo.Snow.ID] = ColorGroup.Snow;
        BlockIndex[BlockInfo.SnowBlock.ID] = ColorGroup.Snow;
        BlockIndex[BlockInfo.ClayBlock.ID] = ColorGroup.Clay;
        BlockIndex[BlockInfo.Dirt.ID] = ColorGroup.Dirt;
        BlockIndex[BlockInfo.Stone.ID] = ColorGroup.Stone;
        BlockIndex[BlockInfo.Cobblestone.ID] = ColorGroup.Stone;
        BlockIndex[BlockInfo.CoalOre.ID] = ColorGroup.Stone;
        BlockIndex[BlockInfo.IronOre.ID] = ColorGroup.Stone;
        BlockIndex[BlockInfo.GoldOre.ID] = ColorGroup.Stone;
        BlockIndex[BlockInfo.DiamondOre.ID] = ColorGroup.Stone;
        BlockIndex[BlockInfo.RedstoneOre.ID] = ColorGroup.Stone;
        BlockIndex[BlockInfo.LapisOre.ID] = ColorGroup.Stone;
        BlockIndex[BlockInfo.Water.ID] = ColorGroup.Water;
        BlockIndex[BlockInfo.StationaryWater.ID] = ColorGroup.Water;
        BlockIndex[BlockInfo.Wood.ID] = ColorGroup.Wood;
    }
    ...
    /// </summary>
    public ColorGroup[] BlockIndex { get; }

    /// <summary>
    ///     Returns the baseline color index associated with the <see cref="ColorGroup" /> currently representing the given
    ///     block ID.
    /// </summary>
    ...
    /// <summary>
    ///     Returns a color index associated with the <see cref="ColorGroup" /> currently representing the given block ID and
    ...
    ///     Thrown when either <paramref name="blockId" /> or
    ///     <paramref name="level" /> are out of their normal ranges.
    /// </exception>
    public int BlockToColorIndex(int blockId, int level)
    {
        if (level < 0 || level >= _groupSize)
            throw new ArgumentOutOfRangeException("level", level,
                "Argument 'level' must be in range [0, " + (_groupSize - 1) + "]");
        if (blockId < 0 || blockId >= 4096) throw new ArgumentOutOfRangeException("blockId");

        return (int)BlockIndex[blockId] * _groupSize + level;
    }

    /// <summary>
    ///     Returns a <see cref="Color" /> value assocated with the <see cref="ColorGroup" /> currently representing the given
    ///     block ID.
    /// </summary>
    /// <param name="blockId">The ID of a block.</param>
    ...
    /// <summary>
    ///     Returns a <see cref="Color" /> value associated with the <see cref="ColorGroup" /> currently representing the given
    ///     block ID and based on the given level.
    /// </summary>
    /// <param name="blockId">The ID of a block.</param>
    /// <param name="level">The color level to select from within the derived <see cref="ColorGroup" />.</param>
    /// <returns>A <see cref="Color" /> value.</returns>
    ...
    {
        var group = colorIndex / _groupSize;
    ...
    /// <exception cref="ArgumentOutOfRangeException">
    ///     Thrown when the <paramref name="level" /> is out of range with respect to
    ///     the current <see cref="ColorGroupSize" /> parameter.
    /// </exception>
    ...
    ///     Thrown when the <paramref name="group" /> and <paramref name="level" /> map
    ///     to an invalid color index.
    /// </exception>
    public Color GroupToColor(ColorGroup group, int level)
    {
        var ci = GroupToColorIndex(group, level);
        if (ci < 0 || ci >= 256)
    ...
    /// <summary>
    ///     Rebuilds the internal color conversion tables.  Should be called after modifying the <see cref="ColorIndex" />
    ///     table.
    /// </summary>
    public void RefreshColorCache()
    {
        for (var i = 0; i < ColorIndex.Length; i++) _labIndex[i] = RgbToLab(ColorIndex[i]);
    }
    ...
    /// <remarks>
    ///     This method performs color comparisons in the CIELAB color space, to find the best match according to human
    ///     perception.
    /// </remarks>
    public int NearestColorIndex(Color color)
    {
        ...
        var cr = RgbToLab(color);

        for (var i = 0; i < ColorIndex.Length; i++)
        {
            if (ColorIndex[i].A == 0) continue;

            var x = cr.X - _labIndex[i].X;
            var y = cr.Y - _labIndex[i].Y;
            var z = cr.Z - _labIndex[i].Z;
    ...
    {
        return ColorIndex[NearestColorIndex(color)];
    }
