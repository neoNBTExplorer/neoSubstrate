...
public class Level : INbtObject<Level>, ICopyable<Level>
{
    ...
    protected Level(Level p)
    {
        ...
        if (p._player != null) _player = p._player.Copy();

        if (p.Source != null) Source = p.Source.Copy() as TagNodeCompound;
    }
    ...
    /// </summary>
    /// <remarks>
    ///     If there is a <see cref="Player" /> object attached to this world, the player's world field
    ...
    public string LevelName
    {
        ...
        {
            _name = value;
            if (_player != null) _player.World = value;
        }
    }
    ...
    public bool Save()
    {
        ...
        try
        {
            var nf = new NBTFile(Path.Combine(_world.Path, "level.dat"));
        ...
        {
            var lex = new LevelIOException("Could not save level file.", ex);
            lex.Data["Level"] = this;
            throw lex;
        }
    }
    ...
    public virtual Level LoadTree(TagNode tree)
    {
        ...
        if (ctree.ContainsKey("Player")) _player = new Player().LoadTree(ctree["Player"]);
        ...
        if (ctree.ContainsKey("version")) _version = ctree["version"].ToTagInt();
        if (ctree.ContainsKey("LevelName")) _name = ctree["LevelName"].ToTagString();

        if (ctree.ContainsKey("generatorName")) GeneratorName = ctree["generatorName"].ToTagString();

        if (ctree.ContainsKey("raining")) _raining = ctree["raining"].ToTagByte();
        if (ctree.ContainsKey("thundering")) _thundering = ctree["thundering"].ToTagByte();
        if (ctree.ContainsKey("rainTime")) _rainTime = ctree["rainTime"].ToTagInt();
        if (ctree.ContainsKey("thunderTime")) _thunderTime = ctree["thunderTime"].ToTagInt();

        if (ctree.ContainsKey("GameType")) _gameType = ctree["GameType"].ToTagInt();
        if (ctree.ContainsKey("MapFeatures")) _mapFeatures = ctree["MapFeatures"].ToTagByte();
        if (ctree.ContainsKey("hardcore")) _hardcore = ctree["hardcore"].ToTagByte();

        if (ctree.ContainsKey("generatorVersion")) _generatorVersion = ctree["generatorVersion"].ToTagInt();
        if (ctree.ContainsKey("generatorOptions")) _generatorOptions = ctree["generatorOptions"].ToTagString();
        if (ctree.ContainsKey("allowCommands")) _allowCommands = ctree["allowCommands"].ToTagByte();
        if (ctree.ContainsKey("initialized")) _initialized = ctree["initialized"].ToTagByte();
        if (ctree.ContainsKey("DayTime")) _DayTime = ctree["DayTime"].ToTagLong();
        if (ctree.ContainsKey("GameRules"))
        {
            var gr = ctree["GameRules"].ToTagCompound();
    ...
    public virtual TagNode BuildTree()
    {
        ...
        if (_player != null) data["Player"] = _player.BuildTree();
        ...
        if (_version != null && _version != 0) data["version"] = new TagNodeInt(_version ?? 0);

        if (_name != null) data["LevelName"] = new TagNodeString(_name);

        if (GeneratorName != null) data["generatorName"] = new TagNodeString(GeneratorName);

        if (_raining != null) data["raining"] = new TagNodeByte(_raining ?? 0);
        if (_thundering != null) data["thundering"] = new TagNodeByte(_thundering ?? 0);
        if (_rainTime != null) data["rainTime"] = new TagNodeInt(_rainTime ?? 0);
        if (_thunderTime != null) data["thunderTime"] = new TagNodeInt(_thunderTime ?? 0);

        if (_gameType != null) data["GameType"] = new TagNodeInt(_gameType ?? 0);
        if (_mapFeatures != null) data["MapFeatures"] = new TagNodeByte(_mapFeatures ?? 0);
        if (_hardcore != null) data["hardcore"] = new TagNodeByte(_hardcore ?? 0);

        if (_generatorOptions != null) data["generatorOptions"] = new TagNodeString(_generatorOptions);
        if (_generatorVersion != null) data["generatorVersion"] = new TagNodeInt(_generatorVersion ?? 0);
        if (_allowCommands != null) data["allowCommands"] = new TagNodeByte(_allowCommands ?? 0);
        if (_initialized != null) data["initialized"] = new TagNodeByte(_initialized ?? 0);
        if (_DayTime != null) data["DayTime"] = new TagNodeLong(_DayTime ?? 0);
        var gr = new TagNodeCompound();
        ...
        if (Source != null) data.MergeFrom(Source);
