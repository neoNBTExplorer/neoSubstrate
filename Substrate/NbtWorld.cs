...
/// <remarks>
///     <para>
///         By default, NbtWorld registers handlers to check if a given world can be opened as an <see cref="AlphaWorld" />
///         or
///         a <see cref="BetaWorld" />, which are used by <see cref="NbtWorld" />'s generic <see cref="Open(string)" />
///         method to automatically
///         detect a world's type and open it.
///     </para>
///     <para>
///         Advanced implementors can support loading other Nbt-compatible world formats by extending
///         <see cref="NbtWorld" /> and registering
///         an event handler with the <see cref="ResolveOpen" /> event, which will allow the generic
///         <see cref="Open(string)" /> method to
...
public abstract class NbtWorld
{
    ...
    {
        if (ResolveOpen == null) return null;

        var eventArgs = new OpenWorldEventArgs(path);
        ResolveOpen(null, eventArgs);

        if (eventArgs.HandlerCount != 1) return null;


        foreach (var callback in eventArgs.Handlers) return callback(path);
    ...
    /// <summary>
    ///     Raised when <see cref="Open" /> is called, used to find a concrete <see cref="NbtWorld" /> type that can open the
    ///     world.
    /// </summary>
