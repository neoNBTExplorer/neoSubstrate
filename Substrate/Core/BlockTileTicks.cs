...
public class BlockTileTicks
{
    ...
    private readonly TagNodeList _tileTicks;
    ...
    public int GetTileTickValue(int x, int y, int z)
    {
        ...
        if (!_tileTickTable.TryGetValue(key, out te)) return 0;
    ...
    public TileTick GetTileTick(int x, int y, int z)
    {
        ...
        if (!_tileTickTable.TryGetValue(key, out te)) return null;

        if (!te.ContainsKey("i")) return null;
    ...
    public void SetTileTick(int x, int y, int z, TileTick te)
    {
        ...
        if (_tileTickTable.TryGetValue(key, out oldte)) _tileTicks.Remove(oldte);
    ...
    public void ClearTileTick(int x, int y, int z)
    {
        ...
        if (!_tileTickTable.TryGetValue(key, out te)) return;
