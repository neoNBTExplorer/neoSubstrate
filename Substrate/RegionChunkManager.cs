...
public class RegionChunkManager : IChunkManager, IEnumerable<ChunkRef>
{
    ...
    /// <summary>
    ///     Creates a new <see cref="RegionChunkManager" /> instance given a backing <see cref="RegionManager" /> and
    ///     <see cref="ChunkCache" />.
    /// </summary>
    ...
    /// </summary>
    /// <param name="cm">
    ///     A <see cref="RegionChunkManager" /> to get a <see cref="RegionManager" /> and
    ///     <see cref="ChunkCache" /> from.
    /// </param>
    public RegionChunkManager(RegionChunkManager cm)
    {
        RegionManager = cm.RegionManager;
        _cache = cm._cache;
    }
    ...
    {
        var src_r = GetRegion(src_cx, src_cz);
        if (src_r == null) return null;

        var dst_r = GetRegion(dst_cx, dst_cz);
        if (dst_r == null)
        {
            var rx = dst_cx >> REGION_XLOG;
            var rz = dst_cz >> REGION_ZLOG;
            dst_r = RegionManager.CreateRegion(rx, rz);
        }

        var c = src_r.GetChunk(src_cx & REGION_XMASK, src_cz & REGION_ZMASK);
    ...
    public void RelightDirtyChunks()
    {
        ...
        var dirty = new Dictionary<ChunkKey, ChunkRef>();

        _cache.SyncDirty();

        var en = _cache.GetDirtyEnumerator();
        while (en.MoveNext())
        {
            var key = new ChunkKey(en.Current.X, en.Current.Z);
            dirty[key] = en.Current;
        }

        foreach (var chunk in dirty.Values)
        {
        ...
        foreach (var chunk in dirty.Values)
        {
            ...
            {
                var east = chunk.GetEastNeighbor();
            ...
            if (!dirty.ContainsKey(new ChunkKey(chunk.X, chunk.Z + 1)))
            {
                var west = chunk.GetWestNeighbor();
            ...
            {
                var north = chunk.GetNorthNeighbor();
            ...
            if (!dirty.ContainsKey(new ChunkKey(chunk.X + 1, chunk.Z)))
            {
                var south = chunk.GetSouthNeighbor();
    ...
    {
        var cx = r.X * REGION_XLEN + lcx;
        var cz = r.Z * REGION_ZLEN + lcz;
    ...
    private IRegion GetRegion(int cx, int cz)
    {
        cx >>= REGION_XLOG;
        cz >>= REGION_ZLOG;
        return RegionManager.GetRegion(cx, cz);
    }


    private class Enumerator : IEnumerator<ChunkRef>
    {
        private readonly RegionChunkManager _cm;

        private readonly IEnumerator<IRegion> _enum;
        ...
        public ChunkRef Current
        {
            get
            {
                if (_x >= REGION_XLEN) throw new InvalidOperationException();
                return _chunk;
            }
        }
        ...
        public virtual bool MoveNext()
        {
            ...
            if (_enum == null) return MoveNextInRegion();

            while (true)
            {
                if (_x >= REGION_XLEN)
                {
                    if (!_enum.MoveNext()) return false;
                    _x = 0;
                    _z = -1;
                    _region = _enum.Current;
                }
    ...
    public ChunkRef CreateChunk(int cx, int cz)
    {
        ...
        if (r == null)
        {
            var rx = cx >> REGION_XLOG;
            var rz = cz >> REGION_ZLOG;
            r = RegionManager.CreateRegion(rx, rz);
        }
    ...
    public int Save()
    {
        ...
        while (en.MoveNext())
        {
            ...
            var r = GetRegion(chunk.X, chunk.Z);
            if (r == null) continue;
            using (var chunkOutStream = r.GetChunkOutStream(chunk.LocalX, chunk.LocalZ))
            {
    ...
    public bool DeleteChunk(int cx, int cz)
    {
        ...
        if (!r.DeleteChunk(cx & REGION_XMASK, cz & REGION_ZMASK)) return false;

        if (r.ChunkCount() == 0) RegionManager.DeleteRegion(r.X, r.Z);
