...
public interface IBoundedLitBlockCollection : IBoundedBlockCollection
{
    ...
    /// <summary>
    ///     Gets the Y-coordinate of the lowest block with unobstructed view of the sky at the given coordinates within a
    ///     bounded block container.
    /// </summary>
    /// <param name="x">The container-local X-coordinate of a block.</param>
    /// <param name="z">The container-local Z-coordinate of a block.</param>
    /// <returns>The height value of an X-Z coordinate pair in the block container.</returns>
    /// <remarks>
    ///     The height value represents the lowest block with an unobstructed view of the sky.  This is the lowest block with
    ///     a maximum-value sky-light value.  Fully transparent blocks, like glass, do not count as an obstruction.
    /// </remarks>
    int GetHeight(int x, int z);

    /// <summary>
    ///     Sets the Y-coordinate of the lowest block with unobstructed view of the sky at the given coordinates within a
    ///     bounded block container.
    /// </summary>
    /// <param name="x">The container-local X-coordinate of a block.</param>
    /// <param name="z">The container-local Z-coordinate of a block.</param>
    ...
    /// <remarks>
    ///     Minecraft lighting algorithms rely heavily on this value being correct.  Setting this value too low may result in
    ///     rooms that can never get dark, for example.
    /// </remarks>
    ...
    ///         If the block is itself a light source, many nearby blocks may be updated to maintain consistent lighting.
    ///         These
    ///         updates may also touch neighboring <see cref="ILitBlockCollection" /> objects, if they can be resolved.
    ///     </para>
    ///     <para>
    ///         This function assumes that the entire <see cref="ILitBlockCollection" /> and neighboring
    ///         <see cref="ILitBlockCollection" />s
    ///         already have consistent lighting, with the exception of the block being updated.  If this assumption is
    ///         violated,
    ...
    ///         <see cref="IBoundedLitBlockCollection" />
    ///         and all neighboring <see cref="IBoundedLitBlockCollection" />s, or lighting may fail to converge correctly.
    ///         This function cannot reset the lighting on its own, due to interactions between
    ///         <see cref="IBoundedLitBlockCollection" />s.
    ///     </para>
    ///     <para>
    ///         If many light source or block opacity values will be modified in this <see cref="IBoundedLitBlockCollection" />
    ///         , it may
    ///         be preferable to avoid explicit or implicit calls to <see cref="UpdateBlockLight" /> and call this function
    ///         once when
    ...
    ///         If many light source or block opacity values will be modified in this <see cref="IBoundedLitBlockCollection" />
    ///         , it may
    ///         be preferable to avoid explicit or implicit calls to <see cref="UpdateSkyLight" /> and call this function once
    ///         when
    ///         modifications are complete.
    ///     </para>
    /// </remarks>
    ...
    /// <summary>
    ///     Reconciles any block-source lighting inconsistencies between this <see cref="IBoundedLitBlockCollection" /> and any
    ///     of its neighbors.
    /// </summary>
    /// <remarks>
    ///     It will be necessary to call this function if an <see cref="IBoundedLitBlockCollection" /> is reset and rebuilt,
    ///     but
    ///     some of its neighbors are not.  A rebuilt <see cref="IBoundedLitBlockCollection" /> will spill lighting updates
    ///     into its neighbors,
    ...
    /// <summary>
    ///     Reconciles any sky-source lighting inconsistencies between this <see cref="IBoundedLitBlockCollection" /> and any
    ///     of its neighbors.
    /// </summary>
    /// <remarks>
    ///     It will be necessary to call this function if an <see cref="IBoundedLitBlockCollection" /> is reset and rebuilt,
    ...
    /// <summary>
    ///     Reconciles any block-source lighting inconsistencies between this <see cref="IBoundedLitBlockCollection" /> and
    ///     another <see cref="IBoundedLitBlockCollection" /> on a given edge.
    /// </summary>
    /// <param name="blockset">
    ///     An <see cref="IBoundedLitBlockCollection" />-compatible object with the same dimensions as this
    ///     <see cref="IBoundedLitBlockCollection" />.
    /// </param>
    ...
    /// <summary>
    ///     Reconciles any sky-source lighting inconsistencies between this <see cref="IBoundedLitBlockCollection" /> and
    ///     another <see cref="IBoundedLitBlockCollection" /> on a given edge.
    /// </summary>
    /// <param name="blockset">
    ///     An <see cref="IBoundedLitBlockCollection" />-compatible object with the same dimensions as this
...
public interface IBoundedPropertyBlockCollection : IBoundedBlockCollection
{
    ...
    /// <param name="z">The container-local Z-coordinate of a block.</param>
    /// <returns>
    ///     A <see cref="TileEntity" /> record attached to a block at the given coordinates, or null if no tile entity is
    ///     set.
    /// </returns>
    ...
    ///     Thrown when a <see cref="TileEntity" /> is added to a block that does not
    ///     use tile entities.
    /// </exception>
    ...
    /// <param name="z">The container-local Z-coordinate of a block.</param>
    /// <exception cref="InvalidOperationException">
    ///     Thrown when a <see cref="TileEntity" /> is created for a block that does
    ///     not use tile entities.
    /// </exception>
    /// <exception cref="UnknownTileEntityException">
    ///     Thrown when the tile entity type associated with the given block has not
    ///     been registered with <see cref="TileEntityFactory" />.
    /// </exception>
...
public interface IBoundedActiveBlockCollection : IBoundedBlockCollection
{
    ...
    /// <param name="z">The container-local Z-coordinate of a block.</param>
    /// <returns>
    ///     A <see cref="TileTick" /> record attached to a block at the given coordinates, or null if no tile entity is
    ///     set.
    /// </returns>
...
public interface IBoundedAlphaBlockCollection : IBoundedDataBlockCollection, IBoundedLitBlockCollection,
    IBoundedPropertyBlockCollection
{
    /// <summary>
