...
/// <remarks>
///     <para>
///         The preferred method to lookup
///         Minecraft block IDs is to access the ID field of the corresponding static BlockInfo
///         object in the BlockInfo class.
///     </para>
///     <para>
///         The static BlockInfo objects can be re-bound to new BlockInfo objects, allowing
...
///         external source and rebinding the objects in BlockInfo.
///     </para>
/// </remarks>
...
/// </summary>
/// <remarks>
///     By default, all known MC block types are already defined and registered, assuming Substrate
...
///     in the <see cref="BlockInfo" /> class.
/// </remarks>
public class BlockInfo
{
    ...
    static BlockInfo()
    {
        ...
        Lava = new BlockInfo(10, "Lava").SetOpacity(0).SetLuminance(MAX_LUMINANCE).SetState(BlockState.FLUID)
            .SetTick(30);
        StationaryLava = new BlockInfo(11, "Stationary Lava").SetOpacity(0).SetLuminance(MAX_LUMINANCE)
            .SetState(BlockState.FLUID).SetTick(10);
        ...
        RedstoneTorchOn = new BlockInfo(76, "Redstone Torch (On)").SetOpacity(0).SetLuminance(7)
            .SetState(BlockState.NONSOLID).SetTick(2);
        ...
        EndPortal = (BlockInfoEx)new BlockInfoEx(119, "End Portal").SetOpacity(0).SetLuminance(MAX_LUMINANCE)
            .SetState(BlockState.NONSOLID);
        ...
        WeightedPressurePlateLight = new BlockInfo(147, "Weighted Pressure Plate (Light)").SetOpacity(0)
            .SetState(BlockState.NONSOLID).SetTick(20);
        WeightedPressurePlateHeavy = new BlockInfo(148, "Weighted Pressure Plate (Heavy)").SetOpacity(0)
            .SetState(BlockState.NONSOLID).SetTick(20);
        RedstoneComparatorInactive = new BlockInfo(149, "Redstone Comparator (Inactive)").SetOpacity(0).SetTick(10);
        RedstoneComparatorActive = new BlockInfo(150, "Redstone Comparator (Active)").SetOpacity(0).SetLuminance(9)
            .SetTick(10);
    ...
    /// </summary>
    public static ICacheTable<BlockInfo> BlockTable => _blockTableCache;

    /// <summary>
    ///     Gets the lookup table for id-to-opacity values.
    /// </summary>
    public static ICacheTable<int> OpacityTable => _opacityTableCache;

    /// <summary>
    ///     Gets the lookup table for id-to-luminance values.
    /// </summary>
    public static ICacheTable<int> LuminanceTable => _luminanceTableCache;
    ...
    /// </summary>
    public int Opacity { get; private set; } = MAX_OPACITY;
    ...
    public int Luminance { get; private set; } = MIN_LUMINANCE;
    ...
    public bool TransmitsLight { get; private set; }

    /// <summary>
    ///     Checks whether the block partially or fully blocks the transmission of light.
    /// </summary>
    public bool ObscuresLight => Opacity > MIN_OPACITY || !TransmitsLight;
    ...
    public bool BlocksFluid { get; private set; } = true;

    /// <summary>
    ///     Gets the block's physical state type.
    /// </summary>
    public BlockState State { get; private set; } = BlockState.SOLID;
    ...
    {
        Opacity = MIN_OPACITY + opacity;
        _opacityTable[ID] = Opacity;
    ...
    {
        if (_dataLimits == null) return true;
...
public class BlockInfoEx : BlockInfo
{
    ...
    /// </summary>
    public string TileEntityName { get; private set; }
