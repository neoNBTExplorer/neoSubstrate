...
{
    private readonly Dictionary<string, TagNode> _tags;
    ...
    /// <summary>
    ///     Copies all the elements of <paramref name="tree" /> into this <see cref="TagNodeCompound" /> if they do not already
    ///     exist.
    /// </summary>
    ...
    public void MergeFrom(TagNodeCompound tree)
    {
        ...
        {
            if (_tags.ContainsKey(node.Key)) continue;
    ...
    public override TagNode Copy()
    {
        ...
        foreach (var item in _tags) list[item.Key] = item.Value.Copy();
        return list;
    }
    ...
    /// <param name="value">
    ///     When the function returns, contains the subnode assicated with the specified key.  If no subnode
    ///     was found, contains a default value.
    /// </param>
    ...
    /// </summary>
    /// <param name="item">
    ///     The <see cref="KeyValuePair{TKey, TVal}" /> structure representing the key and subnode to add to the
    ///     set.
    /// </param>
    ...
    /// </summary>
    /// <param name="item">
    ///     The <see cref="KeyValuePair{TKey, TValue}" /> structure representing the key and subnode to look
    ///     for.
    /// </param>
    ...
    {
        TagNode value;
        if (!_tags.TryGetValue(item.Key, out value)) return false;
        return value == item.Value;
    }

    /// <summary>
    ///     Copies the elements of the <see cref="ICollection{T}" /> to an array of type
    ///     <see cref="KeyValuePair{TKey, TVal}" />, starting at the specified array index.
    /// </summary>
    /// <param name="array">
    ///     The one-dimensional <see cref="Array" /> that is the destination of the subnodes copied. The Array
    ...
    public void CopyTo(KeyValuePair<string, TagNode>[] array, int arrayIndex)
    {
        ...
        if (array.Length - arrayIndex < _tags.Count) throw new ArgumentException();
    ...
    /// </summary>
    /// <param name="item">
    ///     The <see cref="KeyValuePair{TKey, TVal}" /> structure representing the key and value to remove from
    ///     the set.
    /// </param>
    /// <returns>Status indicating whether a subnode was removed.</returns>
